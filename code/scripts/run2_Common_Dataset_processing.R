...
# Directories 
# ----------------------------------------------------------------------	
	parent.dir <- "/Users/ali/Documents/REPOS/seasonal_occupancy/"
	source.dir <- "/Users/ali/Documents/REPOS/seasonal_occupancy/code/functions/"
	data.dir <- paste(parent.dir, "data/BCR23_2016_data/", sep="") 
# ----------------------------------------------------------------------
# Libraries
# ----------------------------------------------------------------------	
...
# ----------------------------------------------------------------------
	...
	# ----------------------------------------------------------------------	
		...
		ttt.index <- 
			...
			!is.na(xxx$PRIMARY_CHECKLIST_FLAG)
		mean(ttt.index)
		sum(ttt.index)
	...
	# ----------------------------------------------------------------------	
		...
		obs.exp$OBSERVER_ID <- as.numeric(obs.exp$OBSERVER_ID)
		erd$X <- join(
			x = erd$X, 
			y = obs.exp, 
			by = "OBSERVER_ID")
		#head(cbind(ttt$OBSERVER_ID, erd$X$OBSERVER_ID), 250)	
		str(erd$X)	
		sum(is.na(erd$X$expertise_score))	
	# ----------------------------------------------------------------------
...
file_loc <- paste(data.dir, output.tag, "_erd_gridcells.RData", sep="")
save(erd, file=file_loc)
